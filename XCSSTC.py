...
from XCSSTCActionSet import *

class XCSSTC:
    def __init__(self):
        self.env = XCSSTCEnvironment()
        self.perf = []
    ...
    def run_experiments(self):
        for exp in range(conf.max_experiments):
            ...
            self.pop = XCSSTCClassifierSet(self.env, self.actual_time)
            self.init()
    ...
    def run_explor(self):
        self.env.set_state()
        self.match_set = XCSSTCMatchSet(self.pop, self.env, self.actual_time)
        self.generate_prediction_array()
        self.select_action()
        self.action_set = XCSSTCActionSet(self.match_set, self.action, self.env, self.actual_time)
    ...
    def run_GA(self):
        if self.actual_time - self.action_set.ts_num_sum() / self.action_set.numerosity_sum() > conf.theta_ga:
            ...
            child1.experience = 0
            child2.experience = 0
            if self.apply_crossover(child1, child2):
    ...
    def apply_crossover(self, cl1, cl2):
        ...
        cond1 = {}
        cond2 = {}
        all_keys = cl1.condition.keys() | cl2.condition.keys()
        for key in all_keys:
            if random.random() < conf.chi:
                if key in cl1.condition:
                    cond2[key] = cl1.condition[key]
                if key in cl2.condition:
                    cond1[key] = cl2.condition[key]
                cross = True
            else:
                if key in cl1.condition:
                    cond1[key] = cl1.condition[key]
                if key in cl2.condition:
                    cond2[key] = cl2.condition[key]                
        cl1.condition = cond1
        cl2.condition = cond2
        return cross
    
    def apply_mutation(self, cl):
        for key in range(len(self.env.get_state())):
            if random.random() < conf.myu:
                if key in cl.condition:
                    del cl.condition[key]
                else:
                    cl.condition[key] = self.env.state[key]
        if random.random() < conf.myu:
    ...
    def file_writer(self, num):
        ...
        for cl in self.pop.cls:
            ...
            for key in cl.condition.keys():
                temp = "Key: " + str(key) + ", Value: " + str(cl.condition[key]) + "\n"
                cond += temp
